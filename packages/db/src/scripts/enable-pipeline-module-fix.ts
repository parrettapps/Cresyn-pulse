/**
 * Enable pipeline module for all tenants (UPDATE existing records)
 *
 * Run with: pnpm tsx src/scripts/enable-pipeline-module-fix.ts
 */
...
async function main() {
  ...
  for (const tenant of allTenants) {
    ...
    if (existingModule.length > 0) {
      ...
      console.log(`  ✅ Pipeline module ENABLED (was ${existingModule[0].enabled ? 'enabled' : 'disabled'})`);
    } else {
      ...
      console.log(`  ✅ Pipeline module created and enabled`);
    }
  }
